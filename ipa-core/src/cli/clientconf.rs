...
use crate::{
    cli::paths::PathExt,
    config::{ClientConfig, HpkeClientConfig, PeerConfig, NetworkConfig},
    error::BoxError,
};
...
fn assert_network_config(config_toml: &Map<String, Value>, config_str: &str) {
    let nw_config = NetworkConfig::from_toml_str(config_str).expect("Can deserialize network config");
